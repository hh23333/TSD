...
class MaskGroupingTransform_pre(MaskTransform):
    ...
    def apply_to_mask(self, masks, **params):
        ...
        for i, part in enumerate(self.parts_names):
            if self.combine_mode == 'sum':
                parts_masks.append(masks[..., [self.parts_map[k] for k in self.parts_grouping[part]]].sum(axis=-1).clip(0, 1))
            else:
                parts_masks.append(masks[..., [self.parts_map[k] for k in self.parts_grouping[part]]].max(axis=-1).clip(0, 1))
        return np.stack(parts_masks, axis=-1)
